...
def VelidateNumber(P):
    return (P.isdigit() or P == "") and len(P) < 2 and P != 0
...
def feelBoard():
    ...
    for i in range(9):
        for j in range(9):
            ...
            if board[i][j] != "":
                cells[i, j].configure("readonli", fg_color="#C8BFBF") # Поменять цвет!
...
def check_solution():
    global seed
    answer = getValues()
    solution = sudoku.Sudoku(seed=seed).solve().board
    if answer == solution:
        print("Нормас")
        tkinter.messagebox.showinfo("Результат", message="Поздравляю, ты прошел игру!")
    else:
        print("Так се")
        tkinter.messagebox.showinfo("Результат", message="К сожалению в вашем решении обнаружен ИИ, попробуйте еще раз(.")
...
btn = CTkButton(app, text="Проверить решение", command=check_solution)
btn.grid(row=10, column=2, columnspan=3, sticky="ew")
